...
- [x] Resolved-config memoisation, invalidated on write. ⚠️ **Ticked before it was true.** Until Phase 3's settings store landed on 2026-09-18 the resolver was bound nowhere and `forget()` was called only by its own test, so nothing invalidated on write — a sweep found the tick. What holds now: one memo per request (`scoped`), dropped automatically by the org, site group and site models' own `saved` and `deleted` events and by a transaction rollback, for exactly the level written and the sites beneath it. The cross-request per-site cache ADR-022 also asks for is **deferred** (ADR-022 amendment)
