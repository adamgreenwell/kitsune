...
- ✅ **Rich text resolves per block, in storage and in the editor.** Issue #39 called this the awkward case and it is awkward in a way this inventory did not anticipate. `RichTextType::toStorage()` stamps `dir="auto"` on every text-bearing block — `p`, `li`, `h2`, `h3`, `h4`, `blockquote`, `pre`, `figcaption` — and deliberately not on `ul`, `ol` or `figure`, because a direction on a container is inherited by children that should each resolve their own: an English first item would drag an Arabic second item left-to-right, which is the per-field failure one level down. An explicit `dir="rtl"` an author wrote is kept rather than overridden, since that says something more specific than `auto`.
  ...
  ⚠️ **And a block authored right now resolves its own direction too, which took a transaction rather than a default** ([#76](https://github.com/adamgreenwell/kitsune/issues/76)). A block that has just been created has nothing stored to preserve — `Entry` stamps `auto` on the way INTO storage — so Arabic typed into a new paragraph rendered in the chrome's direction until the value was saved. ⚠️ A default of `auto` on the attribute closes that and **breaks list rendering**, measured: it also lands on the paragraph *inside* a list item, and `dir="auto"` resolves from an element's text EXCLUDING any descendant that has its own direction, so the seeded list rendered `LI[auto]=ltr` around `P[auto]=rtl` — bullet on the wrong side. A top-level paragraph and one inside a list item are the same node type, so no static default separates them; an `appendTransaction` can, because it sees the parent. It fills a gap and never overrides: a block that already carries a direction is untouched, a list item's paragraph is skipped, and the browser test asserts both — Arabic and English blocks typed live resolve `rtl` and `ltr`, and the seeded list still reads `UL[-] LI[auto] P[-] LI[auto] P[-]`.
