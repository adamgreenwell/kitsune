...
⚠️ **Three measured residuals, stated because a passing suite would otherwise imply they do not exist:**
...
- **A selected multi-select chip fails WCAG AA colour contrast.** Filament renders a multi-select's selections as `fi-badge` in the panel's PRIMARY colour, and the skeleton's primary is `Color::Amber` — measured by axe as amber-600 `#e17100` on amber-50 `#fffbeb` at 12px, **3.08:1** against the 4.5:1 threshold, impact *serious*. Not specific to relations: the chip markup carries `fi-color-primary`, so `Control::Choices` renders identically once a `multi_select` fixture exists. The relation picker is simply the first multi-select in the seed, which is why it surfaced here. Tracked as [#55](https://github.com/adamgreenwell/kitsune/issues/55); the fix is a panel colour decision rather than a renderer change, which is why it is not bundled with the picker.
