...
- Resolution matches a site's **`base_url`**, never its admin slug. The slug is the admin route key (`/admin/{site}`); using it publicly exposed every site at `/{slug}` on every host while leaving a properly configured site unreachable, and dropped `subdomain` entirely. `canonical_host` and `path_prefix` are derived on save, indexed, and unique together — see the ADR-021 amendment.
...
- `base_url` is operator-entered, so its host is **parsed** rather than string-compared: `https://x.test/`, `http://x.test` and a bare `x.test` all name the same host, and a raw `===` fails all three. ⚠️ The bare form is only unambiguous alongside `url_strategy` — `x.test` and `fr` are the same shape — so the derivation takes the strategy, and reads a bare value as a host exactly when the site is addressed by one. Review found the first version guessing "prefix" for every bare value, which made this bullet false for the case it names.
...
- ⚠️ **An OPEN select dropdown reports THREE blocking violations, from one structural cause.** Measured in two states, because the first report of this captured only the weaker one: open with nothing typed gives `[critical] aria-required-children`; open **with results** gives that plus `[critical] aria-required-parent` on an option and `[serious] list` on the `ul`. The cause is single — `role="listbox"` sits on the outer panel, which also contains the search input, so the listbox's children are a `div` and a `ul` rather than options, the options' parent is not the listbox, and the `ul`'s children are not `li`. Under the ARIA pattern a listbox contains only options, with the text input outside it. **Inherited chrome**, confirmed: a plain non-searchable select on the same page renders a native `<select>` and has none of these, so this belongs to the JavaScript combobox Filament builds rather than to anything `FieldValueRenderer` asks for. Tracked as [#62](https://github.com/adamgreenwell/kitsune/issues/62), which records why a speculative Filament bump is not a fix and what to check first.
- ⚠️ **Nothing in the suite audits MID-INTERACTION, and that is the gap that hid the above.** `accessibility.spec.js` and `rtl.spec.js` audit pages at rest; a dropdown has to be *open* for those nodes to exist. It is the same blind spot that hid the badge contrast one step earlier — that needed something **selected**, this needs something **open** — and both were found only by an audit written to put the widget into the state an author actually uses. Whatever fixes #62 needs an open-dropdown audit alongside it, or the next regression here is invisible too.
...
- ✅ **Rich text resolves per block, in storage and in the editor.** Issue #39 called this the awkward case and it is awkward in a way this inventory did not anticipate. `RichTextType::toStorage()` stamps `dir="auto"` on every text-bearing block — `p`, `li`, `h2`, `h3`, `h4`, `blockquote`, `pre`, `figcaption` — and deliberately not on `ul`, `ol` or `figure`, because a direction on a container is inherited by children that should each resolve their own: an English first item would drag an Arabic second item left-to-right, which is the per-field failure one level down. An explicit `dir="rtl"` an author wrote is kept rather than overridden, since that says something more specific than `auto`.

  ⚠️ **The docblock claimed this already worked.** `RichTextType::control()` said *"`dir` is already in ALLOWED_ATTRIBUTES, so per-block direction survives sanitising"* — true, and not the same thing as existing. Nothing PRODUCED a `dir`, so an author writing an Arabic paragraph beside an English one got neither. Surviving is not producing, and the sentence read as though the feature were present.

  ⚠️ **The editor was the residual and [#67](https://github.com/adamgreenwell/kitsune/issues/67) closed it — in TWO places, because the obvious one was the wrong one.** Filament's editor is TipTap/ProseMirror, which drops every attribute its document model does not declare, so the fix looks like a browser extension. A browser extension alone changed nothing, measured twice. Probing the page showed why: what Filament hands the browser is not the stored HTML but a **ProseMirror JSON document**, whose paragraph nodes carried `{"textAlign":"start"}` and no `dir` — the conversion runs in `tiptap-php`, on the server, before any JS could keep the attribute. So `BlockDirectionPlugin` declares `dir` on both sides: `BlockDirection` (PHP) keeps it in the document the browser **receives**, and `rich-editor-direction.js` keeps it in the document the browser **sends back**.

  The editor DOM now carries `<p dir="auto">` and `<li dir="auto">` with `<ul>` undirected, and `e2e/direction.spec.js` measures the resolved direction of each block inside the editor — Arabic `rtl`, English `ltr`, in one field, in an LTR admin. ⚠️ **Filament ships a `textDirection` extension that is deliberately NOT used:** it takes a `direction` option and makes it the *default* for every node type, and a direction invented on a `ul` or `ol` is the per-field failure one level down. The containers ARE declared here with a **null default**, so an author's own `<ul dir="rtl">` — which `Entry` keeps, leaving its items unstamped because they inherit it — survives the editor rather than being replaced by per-item `auto` on the next save. Declaring and defaulting are separate decisions, and only the second is the rule. The node list is `BlockDirectionPlugin::EDITOR_NODES`, asserted against `Entry`'s stamped tags, against the JS file, and against the node names the bundled editor actually declares.

  ⚠️ **And a block authored right now resolves its own direction too, which took a transaction rather than a default** ([#76](https://github.com/adamgreenwell/kitsune/issues/76)). A block that has just been created has nothing stored to preserve — `Entry` stamps `auto` on the way INTO storage — so Arabic typed into a new paragraph rendered in the chrome's direction until the value was saved. A static default of `auto` cannot close it: a top-level paragraph and the one inside a list item are the same node type, and `dir="auto"` resolves from an element's text EXCLUDING any descendant that has its own direction, so `auto` on both halves left `LI[auto]=ltr` wrapping `P[auto]=rtl` — bullet on the wrong side. An `appendTransaction` can tell them apart, because it sees the parent.

  ⚠️ **The rule that holds is one sentence, and getting there took a wrong turn worth recording.** The first version left the paragraph inside a list item undirected *and gave the item nothing either*, so a new bullet still showed the chrome's direction — and the save stored the broken pair anyway, because `tiptap-php` renders an item's text into a `<p>` inside the `<li>` and `Entry` stamped both. Measured in a browser: `<li dir="auto"><p dir="auto">` resolves `li=ltr, p=rtl`, while `<li dir="auto"><p>` resolves `li=rtl, p=rtl`. So: **every text-bearing block resolves its own direction, except the first block inside another block — that one is left undirected, so the block around it resolves from its text.** The item renders the marker, so the item is what has to resolve. `Entry::yieldsToOuterBlock()` is the storage half and the editor extension is the editing half, and the two are the same rule or a save changes what the author was shown.

  It fills a gap and never overrides: a block that already carries a direction is untouched, a generated `auto` is taken back off a block that has come to yield — which is what toggling a list does to the paragraph the author was typing in — and nothing under an author's fixed `ltr`/`rtl` is given one at all. The browser suite asserts all four: blocks typed live resolve `rtl` and `ltr`, the seeded list reads `UL[-] LI[auto] P[-] LI[auto] P[-]`, toggling a list moves the direction onto the item, and the paragraphs inside a `<blockquote dir="rtl">` still inherit it after an edit.

  ⚠️ **Nothing in the app renders rich text read-only,** and that is still true — the public route is the Phase 0 placeholder. What changed is that the editor is now an honest surface to measure: it renders the stored blocks with their directions, so the browser test measures Kitsune's own output rather than the browser's `dir="auto"` implementation.
