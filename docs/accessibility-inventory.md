...
- ✅ **Rich text resolves per block, in storage and in the editor.** Issue #39 called this the awkward case and it is awkward in a way this inventory did not anticipate. `RichTextType::toStorage()` stamps `dir="auto"` on every text-bearing block — `p`, `li`, `h2`, `h3`, `h4`, `blockquote`, `pre`, `figcaption` — and deliberately not on `ul`, `ol` or `figure`, because a direction on a container is inherited by children that should each resolve their own: an English first item would drag an Arabic second item left-to-right, which is the per-field failure one level down. An explicit `dir="rtl"` an author wrote is kept rather than overridden, since that says something more specific than `auto`.
  ...
  ⚠️ **One gap remains inside the editor, and the obvious fix for it is measurably worse.** A block the author has just created — pressing Enter, or the first paragraph of an empty editor — has no stored direction to preserve, because `Entry` stamps `auto` on the way INTO storage. So Arabic typed into a NEW block renders in the chrome's direction until the value is saved. Review asked for a default of `auto` to close it, and the browser said no: `auto` on every text-bearing node also lands on the paragraph **inside** a list item, and `dir="auto"` resolves from an element's text EXCLUDING any descendant that has its own direction — so the seeded list rendered `LI[auto]=ltr` around `P[auto]=rtl`, text flowing right-to-left with the item's own direction left-to-right and the bullet on the wrong side. A top-level paragraph and one inside a list item are the same node type, so no static default separates them. Closing it needs a handler that knows a block's parent, which is its own piece of work — filed as [#76](https://github.com/adamgreenwell/kitsune/issues/76); until then the cost is one save, and the alternative was a visible regression in content that renders correctly today.
