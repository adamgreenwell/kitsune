...
⚠️ **What is not done, and why it is not merely unfinished:** the per-field UI does not exist. `EntryResource` renders `title`, `slug` and `status` directly, so there is no textarea, rich-text editor, select or relation picker to attach `dir` to. When it lands the attribute must land with it — retrofitting means auditing every component, which is the argument ADR-018 makes for the no-bare-strings rule. ADR-029 settles the shape: a field type names a *kind* of control and one renderer builds it, so the attribute has exactly one place to live rather than twelve.

> ⚠️ This paragraph used to say `FieldType::formComponent()` and `tableColumn()` were "on the contract in `field-types.md` and implemented by nothing". They were never on the interface at all — `field-types.md` §3 was a stale design sketch, which is what ADR-029 and `ContractDocumentationTest` exist to stop recurring.
