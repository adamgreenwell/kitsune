...
- ◐ **Rich text now resolves per block in STORAGE, and Filament's editor drops it.** Issue #39 called this the awkward case and it is awkward in a way this inventory did not anticipate. `RichTextType::toStorage()` stamps `dir="auto"` on every text-bearing block — `p`, `li`, `h2`, `h3`, `h4`, `blockquote`, `pre`, `figcaption` — and deliberately not on `ul`, `ol` or `figure`, because a direction on a container is inherited by children that should each resolve their own: an English first item would drag an Arabic second item left-to-right, which is the per-field failure one level down. An explicit `dir="rtl"` an author wrote is kept rather than overridden, since that says something more specific than `auto`.
  ...
  ⚠️ **The editor is the residual, and it is upstream.** Filament's rich editor is TipTap/ProseMirror, which parses the stored HTML into its own document model and re-renders it; its schema does not declare `dir`, so the attribute is dropped on load. Measured in the browser: the Arabic list item inside the editor carries **no `dir` at all** and resolves to the chrome's `ltr`. So the value is correct in the database and correct for any consumer that renders it, and wrong in the one place an author looks while writing it. `e2e/direction.spec.js` asserts the gap rather than assuming it — if a release starts preserving `dir`, that test fails and the failure is the notification, the same reasoning as #62. Tracked as [#67](https://github.com/adamgreenwell/kitsune/issues/67), because a TipTap extension registered into Filament's editor is front-end work rather than a renderer change.

  ⚠️ **And nothing in the app renders rich text read-only,** which is why there is no browser test measuring resolved per-block direction. The editor strips the attribute and the public route is still the Phase 0 placeholder, so the only honest surfaces are the unit tests over `toStorage()` and the measured editor gap. A test that injected the stored HTML into a page and measured it would be testing the browser's `dir="auto"` implementation, not Kitsune's.
