...
⚠️ **Two measured residuals, stated because a passing suite would otherwise imply they do not exist:**
...
⚠️ **What is not done:** the relational leg. `EntryPicker` renders a searchable select with no option source, because `Entry::relatedIdsFor()` and `relationsThrough()` are private — it needs a public accessor and an after-save hook that writes pivot rows with `org_id`, `field_storage_id` and `ordering`. Relations already have a working relation-manager UI with direction applied, so what is missing is the picker rather than the capability.
