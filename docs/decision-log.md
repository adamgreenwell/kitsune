...
- **Deleting a role records the revocation before the cascade takes it.** The rows go in first and inside the same transaction, because afterwards there is no `role_user` left to read them from.
