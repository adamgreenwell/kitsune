...
>
> ⚠️ **A permission check is not a tenancy check, and the policy makes both.** Review found it asking only about the record's `type_handle`: `SiteScope` constrains the query that LOADS an entry and says nothing about the object afterwards, and an instance update or delete writes by primary key without reapplying it. So a record loaded under one site and used after a context switch was authorised by the grants of the site the caller had moved to — and an owner was the worst case, because the bypass answers for the *current* org. The policy asks whether the current scope's own query would return the record (`site_id` matching, or `site_id IS NULL` inside the same org, which is ADR-021's org-shared case) before it resolves any permission at all. A test pins that answer to the query's, shape by shape, because a hand-written copy of a scope's clause that may drift is worse than none.
>
...
>
> ⚠️ **An entry that is ALREADY published keeps that option, whoever is editing it**, and the first version did not — review found the permission turning into a licence to unpublish. `publish` is permission to move an entry *into* the published state; withholding the option outright also withheld the entry's own current value, so a copy-editor could not fix a typo on a published article without demoting or archiving it first. The distinction is the transition rather than the value, and the **stored** status is what decides — so the concession cannot be used to reach the state.
>
> ⚠️ **A relation manager is its own route, so the schema gate has to be asked there too.** `FieldsRelationManager::canViewForRecord()` authorized on org ownership alone, so a non-owner correctly refused the entry-type edit page could still submit that Livewire component and create, edit or delete **fields** — rewriting the schema through the back of the page they had just been refused. Its own docblock already said why in its own words; the predicate was simply not asked. One `mayEditSchema()`, asked twice, rather than two predicates that agree until somebody edits one.
>
> ⚠️ **Schema editing is owner-only in v1.0, and RBAC existing is what made that a requirement.** Before permissions, every member of an org could do everything the tenancy scopes allowed, so an unguarded entry-type builder was consistent; afterwards a copy-editor could still create, rewrite and delete the org's schema while being refused `/c/product`. A permission system that governs the content and not the shape of the content governs the smaller half. There is no `schema.manage` to ask for — the vocabulary above is five actions on **entries** and nothing else, and inventing a subject widens the extension surface that Standing Principle #1 keeps shut until v1.2. **So an org cannot delegate schema editing without making somebody an owner**, which is a real limitation rather than an oversight.
>
> ⚠️ **A policy is not a query scope, and every place that LISTS entries needs the grant applied separately.** `EntryPolicy::view()` is asked about a record somebody already holds; Eloquent never consults it while *building* a query — so the relation picker, the related-records table and the attach dialog each named titles of types the same user is refused at the URL. `Permissions::constrainToViewable()` is that predicate, defined once and applied to all three, because a constraint written three times is two places for it to be forgotten. It hides relations that exist, deliberately: a title is the whole of what those views show, so failing closed is the direction.
>
> ⚠️ **Filament asks `deleteAny`, `forceDeleteAny` and `restoreAny` for bulk actions**, not the singular abilities, and a policy method that does not exist is a denial — including for an owner, because the bypass deliberately lives inside these methods rather than in `Gate::before`. All three are defined even though only one is reachable today: adding a trash filter with restore actions is one line in a Resource, and the failure reads as a broken button rather than as a missing method.
