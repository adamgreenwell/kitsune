...
>
>     ⚠️ **The obvious next question has a different answer, and it is measured rather than assumed.** If an unanchored search multiplies a *run*, does it multiply the **ambiguity ceiling** and the **delimiter proof** too? It multiplies both and neither matters, because both already sit low enough to absorb a factor of the length. At the published ceiling — sixteen copies of `(?:a|a)` then `b`, a product of exactly 65,536 — an all-`a` value measures **178.6 ms unanchored against 0.0 ms anchored** at 5,000 characters; the unanchored comma list `[^,]+(?:,[^,]+)*X` measures **57.5 ms** on 2,500 items, which is already past the ceiling. So the anchoring requirement is scoped to the two-atom allowance, which is the one place the premise was load-bearing. A rule widened by symmetry rather than by measurement would have refused the ordinary list for nothing.
>
...
>
> The remaining `agrees BUT refused` rows are refusals on purpose, not gaps — **27** of them, and each has a reason that is not "nobody got round to it". ⚠️ This paragraph said *"six of them"* and the accounting below said *"of eleven rows"* while the harness reported twenty-three: the count stopped tracking as each structural rule landed, and an accounting that does not add up is worth less than no accounting, because it reads as complete. Every row the harness reports is in the table now, and the arithmetic under it is the check:
>
...
> | `^(?:a*a*b\|a*a*c\|…)$`, `^a*(?:b\|a)*a*c$`, `^a*a*ba*a*c$`, `^()()…(,)(?:,\10?)*X$`, `a*a*b`, `a*a*b$` | Refused on cost, where the harness's subject is short enough to fail fast. The adversarial ones are measured and in the rules above: **6 s**, **20 s**, **1.5 s** — and **60 s** for the last two, which are the unanchored form of the allowance and the reason it now requires `^` |
> | `(?=a)a?a`, `(?:(?=a)a?a)`, `a{0}(?=a)a`, `(?=(?:a))(?:a)?a`, `(?=a+)a?a`, `(?=[0-9])[0-9]?[0-9]`, `(?=a)(?:a?)a`, `(?=a)b*a` | Rule 6, which is **insurance on a version pair this harness does not run** — PCRE 10.44, measured by review. Eight spellings of one redundant shape, and the harness carries all eight because each was published until the round that found it — one of them found by probing the family rather than by review. `^(?=a)b*a` is in the corpus beside the last row and **publishes**, because the anchor is what makes the assertion do work |
...
>
> So of 27 rows: **three** are portability judgements (`\b` and the two `C` categories), **15** are cost refusals — eight where neither engine answers at all and seven where the harness's subject is simply benign — **eight** are rule 6's insurance against PCRE 10.44, and **one** is the over-refusal above. Three plus fifteen plus eight plus one, and only the last is an omission anybody should want closed.
>
