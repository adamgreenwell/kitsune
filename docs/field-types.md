...
>
> **Fresh evidence, measured 2026-09-12.** 236 candidate constructs, enumerated from six independent angles, run through one shared case file so PCRE and ECMAScript are asked the same question. At production fidelity — PCRE compiling `Pattern::delimit()`'s output, ECMAScript compiling the published source — **four constructs the screen accepted diverged, and a fifth made neither engine answer at all**. All four are now refused, so the current count is zero:
>
...
>
>     ⚠️ **A class can divide a run too, which took issue #73's primitive.** The proof read a single literal character, so two obviously disjoint classes could never divide anything: `^[A-Z]{2,3}[0-9]{1,4}[A-Z]{1,2}$` — a UK postcode — was refused for three variable-width atoms in a row, and `[a-z]+[0-9]*` for two. Measured at 1,250 / 2,500 / 5,000 characters, both are **0.03 ms or less at every length**, where the refused `^a*a*a*b$` is 15.4 ms, 119.7 and 949.9: the refusal was a missing question rather than a cost. `atomsCanShareACharacter()` reads both atoms as codepoint **ranges** and intersects them — `[a-m]` and `[n-z]` touch without overlapping and divide, `[a-n]` and `[n-z]` share one character and do not — and it fails closed on anything it cannot read, so `^\p{L}+\p{L}+\p{L}+x$` stays refused.

>     ⚠️ **And "expensive" does not mean "expensive everywhere".** An assertion only does its work where its own body can MATCH, so a branch with a costly leading assertion is engaged there rather than at every position. Nine branches of `(?=(a|a)×13z)A|(?=(b|b)×13z)B|…` are 653 characters carrying 8,192 ambiguous paths each, and they measure **0.11 ms** in Node and **0.24 ms** in PCRE — one subject character engages exactly one of them. Treating them all as engaged everywhere refused that at 73,728 paths, which would block an upgrade over a pattern nothing can make slow.

>     ⚠️ **And a zero-width atom is not free, which that grouping had to learn.** An assertion consumes nothing, so it is not a branch's first character — but it RUNS, at every position the branch is tried. Nine branches of `(?=a*a*b)c|(?=a*a*d)e|…` have disjoint first characters and published, while every lookahead ran anyway: measured at **325.8 ms** on Node and **36.5 ms** on PCRE for nine against 35.9 and 4.1 for one — exactly nine times, on both engines, neither of which hoists the character test in front. A branch whose leading assertion can scan now reports no first characters at all, which shares a subject with everything. A fixed-width assertion body costs nothing to run and is still stepped over.

>     ⚠️ **The same primitive stopped every alternation branch being charged.** Summing them assumes one subject can make them all work: nine branches of `^(?:a*a*b|a*a*c|…)$` really do cost nine times one — **323.3 ms** against 36.0 — because every branch's `a*a*` consumes the same subject, while nine built from different characters cost one, **35.8 ms**. The discriminator is the set of characters each branch can **begin** with, read through everything that can match nothing: `a*a*b` begins with `a` or `b`, which is why it shares a subject with `a*a*c` and not with `c*c*d`. ⚠️ #73's own suggested fix — compare each branch's first REQUIRED atom — would not have separated them, since `b` and `c` differ; the issue is corrected rather than followed.
...
>
>     ⚠️ **The overlap has to be PROVED, and this is where the rule stops.** Either PCRE answers class membership for a single-character lead, or the two atoms are written identically — which is what refuses `(?=[0-9])[0-9]?[0-9]`. So `^(?=[A-Za-z])[A-Za-z0-9]*$` still publishes: the assertion excludes a leading digit where the neighbour admits one, so it is not redundant, and an intersection test that guessed would refuse the identifier pattern every schema has. Deciding whether two **different** classes intersect was the primitive [#73](https://github.com/adamgreenwell/kitsune/issues/73) was open on, and it exists now — `atomsCanShareACharacter()` reads both atoms as codepoint ranges and intersects them. ⚠️ It is deliberately **not** used to widen this rule: rule 6 is insurance against a PCRE version this harness cannot run, so its proofs stay the two certain ones. What the primitive did widen is stated under rule 3, where the cost of being conservative was measured rather than assumed.
>
...
>
> The remaining `agrees BUT refused` rows are refusals on purpose, not gaps — **56** of them, and each has a reason that is not "nobody got round to it". ⚠️ This paragraph said *"six of them"* and the accounting below said *"of eleven rows"* while the harness reported twenty-three: the count stopped tracking as each structural rule landed, and an accounting that does not add up is worth less than no accounting, because it reads as complete. Every row the harness reports is in the table now, and the arithmetic under it is the check:
>
...
> | ~~`^(?:a*a*b\|c*c*d\|…)$` — nine branches, distinct leads~~ | **Closed by [#73](https://github.com/adamgreenwell/kitsune/issues/73), so this row publishes and the table has no over-refusal left.** It was the one known one: eight of the nine branches fail on their first atom, so the real cost is one quadratic — **35.8 ms** against the 324.3 ms of the shared-lead shape beside it, and 36.3 ms for a single branch alone. Summing branch costs could not see that without an atom-against-atom proof |
>
> So of 56 rows: **three** are portability judgements (`\b` and the two `C` categories), **thirty-six** are cost refusals — eight where neither engine answers at all and twenty-eight where the harness's subject is simply benign — and **seventeen** are rule 6's insurance against PCRE 10.44. Three plus thirty-six plus seventeen, and **nothing left over**: the one known over-refusal was closed by #73, so this accounting no longer has a line for an omission anybody should want closed.
>
...
>
> **So the discriminator is proof rather than presence.** A model tells the builder its guarded columns have been derived (`RequiresModelSave::guardedColumnsAreDerived()`), and only the code that derives them can say so — an attribute any caller can supply, a flag it cannot. `FieldStorage` and `GuardedStorageBuilder` were already this mechanism; it is generalised rather than copied a fifth time.
>
> ⚠️ **"A flag it cannot" was written here before it was true of the precedent it cites.** `FieldStorage::$shapeGuarded` was a **public** property when this paragraph named it as the pattern worth generalising, so `$storage->shapeGuarded = true` followed by `$storage->newQuery()->update([…])` was a caller supplying the proof — the one thing the sentence says cannot happen — and it reshaped a *locked* field. Measured, not reasoned. It is private now and answered only through `shapeGuardedFor($builder)`, which also names the save the proof belongs to, so nothing earned before this save can stand for it; `EntryRelation::$guardsRan` was the same defect with the same fix. The principle was right and the page asserted it of code that did not hold it, which is worse than saying nothing: a document that states a consequence becomes the reason nobody goes looking for it. AGENTS.md numbers no invariant for that failure — the nearest are §15, *measure; do not reason* ("if a claim is cheap to verify, verify it"), and §4's amendment, whose lesson is written to generalise past indexes: claiming more for a safety net than it actually compares "is how a guarantee becomes a belief".
>
