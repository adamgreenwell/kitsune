...
>
> The remaining `agrees BUT refused` rows are refusals on purpose, not gaps — **55** of them, and each has a reason that is not "nobody got round to it". ⚠️ This paragraph said *"six of them"* and the accounting below said *"of eleven rows"* while the harness reported twenty-three: the count stopped tracking as each structural rule landed, and an accounting that does not add up is worth less than no accounting, because it reads as complete. Every row the harness reports is in the table now, and the arithmetic under it is the check:
>
...
>
> So of 55 rows: **three** are portability judgements (`\b` and the two `C` categories), **thirty-five** are cost refusals — eight where neither engine answers at all and twenty-seven where the harness's subject is simply benign — and **seventeen** are rule 6's insurance against PCRE 10.44. Three plus thirty-five plus seventeen, and **nothing left over**: the one known over-refusal was closed by #73, so this accounting no longer has a line for an omission anybody should want closed.
>
