...
>
> So of 57 rows: **three** are portability judgements (`\b` and the two `C` categories), **thirty-seven** are cost refusals — eight where neither engine answers at all and twenty-nine where the harness's subject is simply benign — and **seventeen** are rule 6's insurance against PCRE 10.44. Three plus thirty-seven plus seventeen, and **nothing left over**: the one known over-refusal was closed by #73, so this accounting no longer has a line for an omission anybody should want closed.
>
