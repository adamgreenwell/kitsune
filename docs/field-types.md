...
>
> The remaining `agrees BUT refused` rows are refusals on purpose, not gaps — **46** of them, and each has a reason that is not "nobody got round to it". ⚠️ This paragraph said *"six of them"* and the accounting below said *"of eleven rows"* while the harness reported twenty-three: the count stopped tracking as each structural rule landed, and an accounting that does not add up is worth less than no accounting, because it reads as complete. Every row the harness reports is in the table now, and the arithmetic under it is the check:
>
...
> | `(?=a)a?a`, `(?:(?=a)a?a)`, `a{0}(?=a)a`, `(?=(?:a))(?:a)?a`, `(?=a+)a?a`, `(?=[0-9])[0-9]?[0-9]`, `(?=a)(?:a?)a`, `(?=a)b*a`, `(?=a)(?!b)b*a`, `b{0}(?=a)a`, `(?:(?=a))a?a`, `(?=a)^a{0}a`, `(?:(?=a)){2}a{0}a` | Rule 6, which is **insurance on a version pair this harness does not run** — PCRE 10.44, measured by review. Thirteen spellings of one shape, and the harness carries all ten because each was published until the round that found it — one of them found by probing the family rather than by review. `^(?=a)b*a` is in the corpus beside them and **publishes**, because the anchor is what makes the assertion do work |
...
>
> So of 46 rows: **three** are portability judgements (`\b` and the two `C` categories), **29** are cost refusals — eight where neither engine answers at all and twenty-one where the harness's subject is simply benign — **thirteen** are rule 6's insurance against PCRE 10.44, and **one** is the over-refusal above. Three plus twenty-nine plus thirteen plus one, and only the last is an omission anybody should want closed.
>
